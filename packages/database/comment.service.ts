...
import type { Comment } from '@readhub/types'
...
// ─────────────────────────────────────────────────────────────────────────────
// GET COMMENTS COUNT FOR ARTICLES — misma idea que getCommentsCount pero
// para un conjunto de artículos en una sola consulta (usado por
// stats.service.ts para no hacer N round-trips).
// ─────────────────────────────────────────────────────────────────────────────

export async function getCommentsCountForArticles(
  client: Client,
  articleIds: string[]
): Promise<ServiceResult<number>> {
  if (articleIds.length === 0) return { data: 0, error: null }
